...
class TemplateConv(nn.Module):
    ...
    def forward(self, xyzs, features):
        ...
        xyzs = torch.split(tensor=xyzs, split_size_or_sections=1, dim=0)
        xyzs = [torch.squeeze(input=xyz, dim=0).contiguous() for xyz in xyzs]
        ...
        for t in range(nframes):
            # spatial anchor point subsampling by FPS
            xyz = xyzs[t].reshape(1, *xyzs[t].size())
            feature = features[t].reshape(1, *features[t].size()).contiguous()
            anchor_idx = pointnet2_utils.furthest_point_sample(xyz, npoints//self.spatial_stride)                               # (B, N//self.spatial_stride)
            anchor_xyz_flipped = pointnet2_utils.gather_operation(xyz.transpose(1, 2).contiguous(), anchor_idx)                 # (B, 3, N//self.spatial_stride)
            anchor_xyz_expanded = torch.unsqueeze(anchor_xyz_flipped, 3)                                                            # (B, 3, N//spatial_stride, 1)
            anchor_xyz = anchor_xyz_flipped.transpose(1, 2).contiguous()                                                            # (B, N//spatial_stride, 3)
            ...
            xyz_displacement = neighbor_xyz_grouped - anchor_xyz_expanded                                                       # (B, 3, N//spatial_stride, k)
            t_displacement = torch.ones((xyz_displacement.size()[0], 1, xyz_displacement.size()[2], xyz_displacement.size()[3]), dtype=torch.float32, device=device)
            displacement = torch.cat(tensors=(xyz_displacement, t_displacement), dim=1, out=None)                               # (B, 4, N//spatial_stride, k)
            displacement = self.conv_d(displacement)
            
            if self.in_planes != 0:
                neighbor_feature_grouped = pointnet2_utils.grouping_operation(feature, idx)                                 # (B, in_planes, N//spatial_stride, k)
                feature = self.conv_f(neighbor_feature_grouped)
                if self.operator == '+':
                    feature = feature + displacement
                else:
